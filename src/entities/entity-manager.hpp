...
private:
    ...
    struct PoolDeleter {
        template<typename T>
        static void delete_pool(void* p) { delete static_cast<ComponentPool<T>*>(p); }

        void (*fn)(void*) = nullptr;
        void operator()(void* p) const { if (fn) fn(p); }
    };
    std::unordered_map<std::type_index, std::unique_ptr<void, PoolDeleter>> pools_;
...
EntityManager::ComponentPool<T>& EntityManager::pool() {
    ...
    if (it == pools_.end()) {
        ...
        pools_[ti] = std::unique_ptr<void, PoolDeleter>(raw, PoolDeleter{PoolDeleter::delete_pool<T>});
        return *raw;
    }
...
template <typename T>
T& EntityManager::add_component(EntityId id, T component) {
    auto& p = pool<T>();
...
template <typename T>
T* EntityManager::get_component(EntityId id) {
    auto& p = pool<T>();
...
template <typename T>
bool EntityManager::has_component(EntityId id) const {
    auto ti = std::type_index(typeid(T));
    auto it = pools_.find(ti);
