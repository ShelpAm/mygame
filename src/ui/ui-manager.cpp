...
void UIManager::render(const WorldState& world_state, const App& app) {
    ...
    if (app.show_multiplayer()) {
        if (!chat_active_) { chat_buf_[0] = '\0'; chat_active_ = true; }
        render_multiplayer_menu(app);
    } else {
        if (chat_active_) { chat_active_ = false; chat_buf_[0] = '\0'; }
    }
...
void UIManager::render_hud(const WorldState& world_state, const App& app) {
    ...
    ImGui::Text("%s: %d | %s: %s | %s: %.1f",
        loc.get("hud.day").c_str(), world_state.day(),
...
void UIManager::render_dialogue(const App& app) {
    const auto& ds = app.game_mode().dialogue();
    const auto& loc = app.locale();
    ...
    for (const auto& line : ds.history) {
        ...
        if (line.speaker == DialogueLine::player) {
            ...
            const char* name = line.npc_name.empty() ? ds.npc_name.c_str() : line.npc_name.c_str();
    ...
    // Topics
    for (const auto& topic_id : ds.available_topics) {
        std::string label;
        if (topic_id == "__quest_turnin__") label = "- " + loc.get("quest.turnin_label");
        else if (topic_id == "__quest__") label = "- " + loc.get("quest.label");
        else if (topic_id == "__attack__") label = "- " + loc.get("dialogue.attack");
        else {
            label = "- " + loc.get("topic." + topic_id);
            if (label == "- ") label = "- " + topic_id;
        }
    ...
    if (!ds.available_actions.empty()) {
        ...
        for (const auto& action : ds.available_actions) {
...
void UIManager::render_journal(const App& app) {
    const auto& loc = app.locale();
...
void UIManager::render_inventory(const App& app) {
    const auto& loc = app.locale();
...
void UIManager::render_help_panel(const App& app) {
    const auto& loc = app.locale();
...
void UIManager::render_load_menu(const App& app) {
...
void UIManager::render_multiplayer_menu(const App& app) {
    if (!app.show_multiplayer()) return;
    ...
    if (!show) const_cast<App&>(app).set_show_multiplayer(false);

    const auto& loc = app.locale();
    auto* net = const_cast<App&>(app).network_mut();
    if (!net->is_connected()) {
        ...
        if (ImGui::Button(loc.get("mp.connect").c_str())) {
            try { net->connect(host_ip_); } catch (...) {}
            std::string ip(host_ip_);
            if (std::find(server_list_.begin(), server_list_.end(), ip) == server_list_.end()) {
                server_list_.push_back(ip); save_server_list();
            }
        }
        if (!server_list_.empty()) {
            ...
            for (int i = 0; i < (int)server_list_.size(); ++i) {
                ...
                if (ImGui::Button(server_list_[i].c_str())) {
                    strncpy(host_ip_, server_list_[i].c_str(), sizeof(host_ip_) - 1);
                    try { net->connect(server_list_[i]); }
                    catch (...) { /* connection failed, ignore */ }
                }
                ImGui::SameLine();
                if (ImGui::Button("X")) { server_list_.erase(server_list_.begin() + i); save_server_list(); ImGui::PopID(); break; }
                ImGui::PopID();
            }
        }
    } else {
        ...
        ImGui::Text("%s: %zu", loc.get("mp.remote_entities").c_str(), net->remote_entities().size());
        ImGui::Separator();
        ...
        ImGui::EndChild();
        ImGui::InputText("##chat", chat_buf_, sizeof(chat_buf_), ImGuiInputTextFlags_EnterReturnsTrue);
        if (ImGui::IsItemDeactivatedAfterEdit() || ImGui::IsKeyPressed(ImGuiKey_Enter)) {
            if (strlen(chat_buf_) > 0) { const_cast<NetworkManager*>(net)->send_chat(chat_buf_); chat_buf_[0] = '\0'; ImGui::SetKeyboardFocusHere(-1); }
        }
        ImGui::SameLine();
        if (ImGui::Button(loc.get("mp.send").c_str())) {
            if (strlen(chat_buf_) > 0) { const_cast<NetworkManager*>(net)->send_chat(chat_buf_); chat_buf_[0] = '\0'; }
        }
        if (ImGui::Button(loc.get("mp.disconnect").c_str())) net->disconnect();
    }
    ImGui::Separator();
    if (ImGui::Button(loc.get("mp.close").c_str())) const_cast<App&>(app).set_show_multiplayer(false);
    ImGui::End();
}
...
void UIManager::load_server_list() {
    ...
    while (std::getline(f, line)) {
        if (!line.empty()) server_list_.push_back(line);
    }
}

void UIManager::save_server_list() {
    std::ofstream f("saves/servers.txt");
    for (const auto& ip : server_list_) f << ip << '\n';
}

void UIManager::render_map(const App& app) {
    const auto& loc = app.locale();
