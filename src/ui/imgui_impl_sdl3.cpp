...
void ImGui_ImplSDL3_NewFrame() {
    ...
    io.AddKeyEvent(ImGuiMod_Ctrl, keys[SDL_SCANCODE_LCTRL] || keys[SDL_SCANCODE_RCTRL]);
    io.AddKeyEvent(ImGuiMod_Shift, keys[SDL_SCANCODE_LSHIFT] || keys[SDL_SCANCODE_RSHIFT]);
    io.AddKeyEvent(ImGuiMod_Alt, keys[SDL_SCANCODE_LALT] || keys[SDL_SCANCODE_RALT]);
    io.AddKeyEvent(ImGuiMod_Super, keys[SDL_SCANCODE_LGUI] || keys[SDL_SCANCODE_RGUI]);
}
...
bool ImGui_ImplSDLRenderer3_Init(SDL_Renderer* renderer) {
    ...
    g_FontTexture = SDL_CreateTexture(renderer,
    ...
    for (int n = 0; n < drawData->CmdListsCount; n++) {
        ...
        for (int i = 0; i < cmdList->CmdBuffer.Size; i++) {
            ...
            SDL_RenderGeometryRaw(renderer,
                tex,
                xy.data(), (int)sizeof(float) * 2,
                colors.data(), (int)sizeof(SDL_FColor),
                uv.data(), (int)sizeof(float) * 2,
