...
public:
    ...
    const SurvivalState& state() const { return state_; }

    bool is_starving() const { return state_.food <= 0.f; }
    bool is_dehydrated() const { return state_.water <= 0.f; }
    bool is_dead() const { return state_.health <= 0.f; }
