...
void RumorPropagator::update(
    int current_day, std::unordered_map<std::string, NPCState> &all_npcs)
{
    ...
    for (int i = 0; i < exchanges; ++i) {
        ...
        simulate_exchange(all_npcs[npcIds[a]], all_npcs[npcIds[b]],
                          current_day);
        simulate_exchange(all_npcs[npcIds[b]], all_npcs[npcIds[a]],
                          current_day);
    }
}
                                        ...
                                        int current_day)
{
    for (auto const &[fact_id, known] : giver.knowledge) {
        ...
        if (known.confidence < 30 && std::rand() % 100 < 70)
            continue;
        ...
        if (!alreadyKnown) {
            ...
            newFact.confidence =
                std::max(0, known.confidence - (int)(distortion * 30));
            ...
            if (distortion > 0.5f) {
                // Corrupt the version
                newFact.npc_version =
                    known.npc_version + " ...or so I've heard";
            }
            else {
...
bool RumorPropagator::can_reach(NPCState const &npc,
                                std::string const &fact_id) const
{
    return npc.knowledge.contains(fact_id);
}

float RumorPropagator::apply_distortion(std::string const &fact_id,
                                        int hop_count) const
{
