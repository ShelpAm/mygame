...
public:
    ...
    Fact* fact_mutable(const std::string& id);
    void add_or_update_fact(Fact fact);
    bool has_fact(const std::string& id) const;

    void add_relation(const std::string& fact_a, const std::string& fact_b);
    std::vector<std::string> related_facts(const std::string& id) const;

    bool is_topic_known(const std::string& topic) const;
    void mark_topic_known(const std::string& topic);
    const std::unordered_set<std::string>& known_topics() const { return known_topics_; }

    std::vector<const Fact*> facts_witnessed() const;
    std::vector<const Fact*> facts_heard() const;
