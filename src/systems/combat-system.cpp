...
void CombatSystem::resolve_combat(EntityManager& entities, float dt) {
    ...
    for (auto attacker_id : combatants) {
        ...
        if (atkStats->cooldown_remaining > 0.f) continue;

        // Find nearest enemy
        Team enemy_team = (atkStats->team == Team::player) ? Team::enemy : Team::player;
        ...
        events_.push_back({attacker_id, targetId, "attacker", "defender", dmg, killed});
    }
}
...
void CombatSystem::process_soldier_ai(EntityManager& entities) {
    ...
    for (auto id : entities.all_entities()) {
        ...
        auto enemyId = find_nearest_enemy(entities, id,
        ...
        pos->tile_pos = {(int)(pos->world_pos.x / 64.f), (int)(pos->world_pos.y / 64.f)};
    }
}
...
EntityId CombatSystem::find_nearest_enemy(EntityManager& entities,
    ...
    for (auto id : entities.all_entities()) {
        if (id == self) continue;
        auto* cs = entities.get_component<CombatStats>(id);
        if (!cs || !cs->alive || cs->team != enemy_team) continue;
...
void CombatSystem::spawn_enemy_wave(EntityManager& entities, int count,
    ...
    for (int i = 0; i < count; ++i) {
        ...
        entities.add_component<Position>(eid, Position{
            .world_pos = {x, y},
            .tile_pos = {(int)(x / 64.f), (int)(y / 64.f)},
            .z_order = 0.5f
        });
        entities.add_component<Sprite>(eid, Sprite{
            .origin = {12.f, 12.f},
            .color = {0.9f, 0.2f, 0.1f, 1.f},
            .scale = 1.f, .visible = true
        });
        entities.add_component<CombatStats>(eid, CombatStats{
            .team = team, .max_hp = 8, .hp = 8,
