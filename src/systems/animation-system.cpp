    ...
    world.query<Animation, Sprite>().each(
        [delta_time, &resources](flecs::entity, Animation &anim, Sprite &s) {
            ...
            if (anim.frame_timer <= 0) {
                ++anim.frame_index;
                if (anim.frame_index >= anim.clip->frame_textures.size()) {
                    if (anim.looping)
                        anim.frame_index = 0;
                    else
                        anim.frame_index = anim.clip->frame_textures.size() - 1;
                }
            ...
            // frame_textures[frame_index] is now a sprite name.
            s.texture_name = anim.clip->frame_textures[anim.frame_index];
