...
void GameMode::init_world(WorldState& ws, KnowledgeGraph& kg, DialogueEngine& de,
    ...
    ws_ = &ws; kg_ = &kg; de_ = &de; tr_ = &tr; rt_ = &rt;
    cs_ = &cs; qm_ = &qm;

    // Player
    player_entity_ = spawn_player(0, 0);
    ...
    try {
        ...
        for (const auto& item : json.as_object().at("events").as_array()) {
            ...
            if (obj.contains("location_id")) ev.location_id = std::string(obj.at("location_id").as_string());
            if (obj.contains("source_faction_id")) ev.source_faction_id = std::string(obj.at("source_faction_id").as_string());
            if (obj.contains("target_faction_id")) ev.target_faction_id = std::string(obj.at("target_faction_id").as_string());
            if (obj.contains("power_shift")) ev.power_shift = static_cast<int>(obj.at("power_shift").as_int64());
            if (obj.contains("wealth_shift")) ev.wealth_shift = static_cast<int>(obj.at("wealth_shift").as_int64());
            if (obj.contains("cohesion_shift")) ev.cohesion_shift = static_cast<int>(obj.at("cohesion_shift").as_int64());
            auto ts = std::string(obj.at("type").as_string());
            if (ts == "Battle") ev.type = GameEvent::Type::battle;
            else if (ts == "TradeDeal") ev.type = GameEvent::Type::trade_deal;
            else if (ts == "Betrayal") ev.type = GameEvent::Type::betrayal;
            else if (ts == "NaturalDisaster") ev.type = GameEvent::Type::natural_disaster;
            else if (ts == "DiplomaticShift") ev.type = GameEvent::Type::diplomatic_shift;
            else if (ts == "RefugeeWave") ev.type = GameEvent::Type::refugee_wave;
...
EntityId GameMode::spawn_player(float x, float y) {
    ...
    em_.add_component<Sprite>(eid, Sprite{"player", {}, {16, 16}, {0.3f, 0.8f, 0.3f, 1.f}, 1.f, true});
    em_.add_component<CombatStats>(eid, CombatStats{Team::player, 20, 20, 4, 3, 80.f});
    return eid;
}

void GameMode::spawn_npc(const std::string& id, const std::string& name,
                         float x, float y, const std::string& personality,
                         const std::vector<NPCKnowledgeEntry>& known_facts) {
    auto eid = em_.create_entity();
    em_.add_component<Position>(eid, Position{{x, y}, {static_cast<int>(x/64.f), static_cast<int>(y/64.f)}, 1.f});
    ...
    NPCState npc;
    npc.npc_id = id; npc.display_name = name; npc.personality = personality;
    for (const auto& kf : known_facts)
        npc.knowledge[kf.fact_id] = {kf.fact_id, kf.version, kf.confidence, kf.witnessed, kf.source};
    em_.add_component<NPCState>(eid, npc);
    em_.add_component<CombatStats>(eid, CombatStats{Team::neutral, 15, 15, 2, 1, 60.f});
    rt_->set_relation(id, {});
    npc_entities_.push_back(eid);
}

EntityId GameMode::spawn_soldier(EntityId leader, int index, const Vec2f& facing) {
    ...
    em_.add_component<Sprite>(eid, Sprite{"", {}, {12,12}, {0.3f, 0.5f, 0.9f, 1.f}, 0.8f, true});
    em_.add_component<CombatStats>(eid, CombatStats{Team::player, 12, 12, 3, 2, 80.f});
...
void GameMode::spawn_guards(EntityId captain_eid, int count, Team team) {
    ...
    for (int g = 0; g < count; ++g) {
        ...
        em_.add_component<Sprite>(eid, Sprite{"", {}, {10,10}, col, 0.7f, true});
        em_.add_component<CombatStats>(eid, CombatStats{team, 10, 10, 3, 2, 70.f});
        em_.add_component<SoldierAI>(eid, SoldierAI{captain_eid, {gp.x-center.x, gp.y-center.y}, 32.f, 180.f});
    }
}
void GameMode::handle_interaction(LocaleManager& loc) {
    if (dialogue_->active) { end_dialogue(); return; }
    auto* pp = em_.get_component<Position>(player_entity_);
    if (!pp) return;
    auto eid = find_nearest_interactable(pp->world_pos);
    ...
    dialogue_->npc_entity = eid; dialogue_->npc_id = npc->npc_id; dialogue_->npc_name = npc->display_name;
    dialogue_->history.clear(); dialogue_->available_topics.clear(); dialogue_->available_actions.clear();
    dialogue_->can_gift = false; dialogue_->can_threaten = false;
    dialogue_->history.push_back({DialogueLine::npc, "", resp.text, true, npc->display_name});
    dialogue_->npc_trust = trust;
    for (const auto& [tid, _] : npc->knowledge) dialogue_->available_topics.push_back(tid);
    for (const auto& t : kg_->known_topics()) if (!npc->knowledge.contains(t)) dialogue_->available_actions.push_back("tell:" + t);
    if (npc->personality == "hostile") { dialogue_->available_topics.push_back("__attack__"); dialogue_->can_threaten = true; }
    dialogue_->can_gift = true;
    for (const auto* q : qm_->available_quests()) if (q->giver == npc->npc_id) { dialogue_->available_topics.push_back("__quest__"); break; }
    for (const auto* q : qm_->active_quests()) if (q->giver == npc->npc_id) { dialogue_->available_topics.push_back("__quest_turnin__"); break; }
}

EntityId GameMode::find_nearest_interactable(Vec2f player_pos) const {
    EntityId nearest = invalid_entity; float nearestDist = 80.f;
    for (auto eid : em_.all_entities()) {
        if (eid == player_entity_) continue;
        if (!em_.get_component<Interactable>(eid)) continue;
...
void GameMode::do_dialogue_action(const std::string& action, LocaleManager& loc) {
    if (!dialogue_->active) return;
    auto eid = dialogue_->npc_entity;
    auto* npc = em_.get_component<NPCState>(eid);
    if (!npc) return;

    auto addHistory = [&](DialogueLine::Speaker s, const std::string& key, const std::string& raw = "", bool use_raw = false) {
    ...
    if (action == "__gift__") {
        rt_->modify_trust(npc->npc_id, 8); dialogue_->npc_trust += 8;
    ...
    if (action == "__threaten__") {
        rt_->modify_fear(npc->npc_id, 15); dialogue_->npc_fear += 15;
    ...
    if (action == "__quest__") {
        for (auto* q : qm_->available_quests()) {
            if (q->giver != npc->npc_id) continue;
    ...
    if (action.starts_with("tell:")) {
        ...
        if (!known) { rt_->modify_trust(npc->npc_id, 5); dialogue_->npc_trust += 5; }
        return;
    }
    ...
    auto resp = de_->generate_ask_response(*npc, action, dn.empty() ? action : dn, trust);
    ...
    if (resp.trust_delta != 0) { rt_->modify_trust(npc->npc_id, resp.trust_delta); dialogue_->npc_trust += resp.trust_delta; }
    if (resp.is_truthful && !resp.fact_id.empty() && npc->knowledge.contains(action)) {
        ...
        f.origins.push_back({Fact::Origin::Source::npc_testimony, npc->npc_id, "", ws_->day(), npc->knowledge[action].confidence});
        kg_->add_or_update_fact(f);
    }
}

void GameMode::end_dialogue() { *dialogue_ = {}; }

void GameMode::update(float dt, InputManager& input, LocaleManager& loc, WorldState& ws) {
    (void)loc;
    auto* cs = em_.get_component<CombatStats>(player_entity_);
    ...
    if (!is_client) {
        ...
        if (!dead && !dialogue_->active) {
            if (input.is_pressed(InputManager::Action::move_up)) my -= 1;
            if (input.is_pressed(InputManager::Action::move_down)) my += 1;
            if (input.is_pressed(InputManager::Action::move_left)) mx -= 1;
            if (input.is_pressed(InputManager::Action::move_right)) mx += 1;
        }
        auto* pos = em_.get_component<Position>(player_entity_);
        if (pos && !dead && (mx != 0 || my != 0)) {
            float len = std::hypot(mx, my);
            Vec2f dir{mx / len, my / len};
            pos->world_pos.x += dir.x * 200.f * dt;
            pos->world_pos.y += dir.y * 200.f * dt;
            pos->tile_pos = {static_cast<int>(pos->world_pos.x / 64.f), static_cast<int>(pos->world_pos.y / 64.f)};
            ws_->reveal_radius(pos->tile_pos, 8);
            int si = 0;
            for (auto eid : em_.all_entities()) {
                auto* ai = em_.get_component<SoldierAI>(eid);
                if (!ai || ai->follow_target != player_entity_) continue;
                const int PER_ROW[] = {3, 4, 5};
                int rem = ++si, row = 0, col = 0;
                for (row = 0; row < 3; ++row) { if (rem <= PER_ROW[row]) { col = rem - 1; break; } rem -= PER_ROW[row]; }
                if (row >= 3) { row = 2; col = si - PER_ROW[0] - PER_ROW[1] - 1; }
                float s = 36.f;
                ai->formation_offset = {((col - (PER_ROW[row] - 1) / 2.f) * s) * dir.y + (-(50.f + row * 45.f)) * dir.x,
    ...
    if (dead && dialogue_->active) end_dialogue();
}
