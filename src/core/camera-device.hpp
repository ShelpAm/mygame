...
class CameraDevice {
  ...
  public:
    ...
    CameraDevice()
    {
        ...
        if (devcount == 0) {
            SDL_free(devices);
            throw std::runtime_error("Couldn't find any camera devices! Please connect a camera "
                                     "and try again.");
        }
        ...
        for (int i = 0; i < devcount; ++i) {
            int specCount = 0;
            // 获取该设备支持的硬件参数列表
            SDL_CameraSpec **specs = SDL_GetCameraSupportedFormats(devices[i], &specCount);
            ...
            if (specs && specCount > 0) {
                ...
                SDL_free(static_cast<void *>(specs));
            }
        ...
        if (camera == nullptr)
            throw std::runtime_error("Couldn't open any camera: " + std::string(SDL_GetError()));
    ...
    CameraDevice &operator=(CameraDevice const &) = delete;
    ...
    void update(SDL_Renderer *renderer)
    {
        ...
        if (frame != nullptr) {
            ...
            if (!texture) {
                ...
                // 动态创建匹配当前格式的流式纹理
                texture = SDL_CreateTexture(renderer, frame->format, SDL_TEXTUREACCESS_STREAMING,
                                            width, height);
            }
    ...
    bool is_ready() const { return texture != nullptr; }
};
